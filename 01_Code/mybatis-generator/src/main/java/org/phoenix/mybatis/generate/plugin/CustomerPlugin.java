...
public class CustomerPlugin extends PluginAdapter {
	...
	private void generateModel(IntrospectedTable introspectedTable, TopLevelClass topLevelClass) {
		// //
		...
		//
		// // 添加字段的输出代码；
		// sb.append("sb.append(\"").append(", ").append(property) //$NON-NLS-1$
		// //$NON-NLS-2$
		// .append("=\")").append(".append(").append(property) //$NON-NLS-1$
		// //$NON-NLS-2$
		// .append(");"); //$NON-NLS-1$
		//
	...
	public boolean clientGenerated(Interface interfaze, TopLevelClass topLevelClass,
		...
		FullyQualifiedJavaType selectType = new FullyQualifiedJavaType("org.phoenix.mybatis.criterion.SelectCriterion");
		FullyQualifiedJavaType countType = new FullyQualifiedJavaType("org.phoenix.mybatis.criterion.CountCriterion");
		FullyQualifiedJavaType updateType = new FullyQualifiedJavaType("org.phoenix.mybatis.criterion.UpdateCriterion");
		FullyQualifiedJavaType deleteType = new FullyQualifiedJavaType("org.phoenix.mybatis.criterion.DeleteCriterion");
		...
		method.setName("listByCriterion");
		...
		method.addParameter(new Parameter(updateType, "criterion", "@Param(\"criterion\")"));
		interfaze.addMethod(method);
		...
		for (Method m : methods) {
			if (m.getName().startsWith("select")) {
				if (m.getReturnType().getShortNameWithoutTypeArguments().equals("List")) {
					m.setName(m.getName().replaceFirst("select", "list"));
				} else {
					m.setName(m.getName().replaceFirst("select", "get"));
				}
			}
		}
	...
	private void selectSql(Document document, IntrospectedTable introspectedTable) {
		...
		foreachElement.addAttribute(new Attribute("collection", "orWhereClausesList"));
		...
		foreachElement.addAttribute(new Attribute("collection", "whereclauses.andWhereClauselist"));
		foreachElement.addAttribute(new Attribute("item", "whereclause"));
		foreachElement.addElement(
				new TextElement("and ${whereclause.fieldName} ${whereclause.condition} #{whereclause.fieldValue}"));
	}

	private void updateSql(Document document, IntrospectedTable introspectedTable) {
		XmlElement answer = new XmlElement("sql");
		...
		foreachElement.addAttribute(new Attribute("collection", "criterion.orWhereClausesList"));
		foreachElement.addAttribute(new Attribute("item", "whereclauses"));
		foreachElement.addAttribute(new Attribute("separator", "or"));
		XmlElement trimElement = new XmlElement("trim");
		foreachElement.addElement(trimElement);
	...
	private void select(Document document, IntrospectedTable introspectedTable) {
		...
		answer.addAttribute(new Attribute("id", "listByCriterion"));
		answer.addAttribute(new Attribute("parameterType", "org.phoenix.mybatis.criterion.SelectCriterion"));
		...
		whenElement.addAttribute(new Attribute("test", "_parameter != null and selectiveColumnlist.size() != 0"));
		XmlElement foreachElement = new XmlElement("foreach");
		whenElement.addElement(foreachElement);

		foreachElement.addAttribute(new Attribute("collection", "selectiveColumnlist"));
		...
		includeElement.addAttribute(new Attribute("refid", "Criterion_Where_Clause"));

		ifElement = new XmlElement("if");
		answer.addElement(ifElement);
		ifElement.addAttribute(new Attribute("test", "orderByClause != null and orderByClause != ''"));
		ifElement.addElement(new TextElement("order by ${orderByClause}"));

		ifElement = new XmlElement("if");
		answer.addElement(ifElement);

		ifElement.addAttribute(new Attribute("test", "_parameter != null and length != 0"));
	...
	private void count(Document document, IntrospectedTable introspectedTable) {
		...
		answer.addAttribute(new Attribute("parameterType", "org.phoenix.mybatis.criterion.CountCriterion"));
	...
	private void update(Document document, IntrospectedTable introspectedTable) {
		...
		for (IntrospectedColumn introspectedColumn : introspectedTable.getAllColumns()) {
			...
			ifElement.addAttribute(new Attribute("test", introspectedColumn.getJavaProperty("record.") + " != null"));
			ifElement.addElement(
					new TextElement(MyBatis3FormattingUtilities.getAliasedEscapedColumnName(introspectedColumn) + " = "
		...
		includeElement.addAttribute(new Attribute("refid", "Update_Criterion_Where_Clause"));
	}
	...
	private void delete(Document document, IntrospectedTable introspectedTable) {
		...
		answer.addAttribute(new Attribute("parameterType", "org.phoenix.mybatis.criterion.DeleteCriterion"));
